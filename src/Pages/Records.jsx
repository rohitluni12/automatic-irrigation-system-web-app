...
const Records = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const val = [];
    if (user && user[0].email === "admin1@gmail.com") {
      onValue(ref(db, "CurrencyInfo/"), (snapshot) => {
        console.log("admin");
      ...
      onValue(ref(db, "CurrencyInfo/" + userId), (snapshot) => {
        // snapshot.forEach((childSnapshot) => {
        //   console.log(childSnapshot.val())
        //   val.push({
        //     key: childSnapshot.key,
          ...
          {data &&
            data.map((item) => {
              ...
              return Object.entries(data).map(([date, values]) => {
                const { NoteNumber, Price, Latitude, Longitude } = values;
                return (
                  <tr
                    key={uid + "-" + date}
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{Latitude}</td>
                    <td className="px-6 py-4">{Longitude}</td>
                    <td className="px-6 py-4">{NoteNumber}</td>
                    <td className="px-6 py-4">{Price}</td>
                    <td className="px-6 py-4 text-right">
                      <button
                        onClick={(e) => showData(values)}
                        data-hs-overlay="#hs-sign-out-alert"
                        className="p-2 px-4 font-medium bg-teal-500 rounded-md text-slate-50 hover:bg-teal-600"
                      >
                        Edit
                      </button>
                    </td>
          ...
          <div className="relative flex flex-col bg-white shadow-lg rounded-xl dark:bg-gray-800">
            ...
            <div className="p-4 overflow-y-auto text-center sm:p-10">
              ...
              <h3 className="mb-2 text-2xl font-bold text-gray-800 dark:text-gray-200">
                {modalData.NoteNumber}
              </h3>
              <p className="text-gray-500">Latitude : {modalData.Latitude}</p>
              <p className="text-gray-500">Longitude : {modalData.Longitude}</p>
              <p className="text-gray-500">Price : {modalData.Price}</p>
